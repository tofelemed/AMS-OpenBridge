...
- **Audit trail (R19):** display-service now emits governance events (`DISPLAY_CREATED/UPDATED/PUBLISHED/DELETED/SHARED/…`) to Kafka `traverse.cpa.audit-events`; audit-service consumes them into its immutable hash-chained store and exposes `GET /api/v1/audit` (filter by entity/actor/type/time). Compose gains `Kafka__BootstrapServers`.
...
- Audit trail: reuse the existing (currently disconnected) `display-events` Redis channel and `traverse.cpa.audit-events` Kafka topic — connect them, don't invent a third path.
...
| 5.7 | **Audit trail** of display changes: display-service → Kafka `traverse.cpa.audit-events`; `GET /api/v1/audit` query | R19, W1 | ✅ |
...
- **Flink execution job (7.2):** new `AnalysisExecutionJob.java` **consumes `traverse.analysis.executions`** (dead before — audit §7.3) and evaluates the expression with a dependency-free shunting-yard `ExpressionEvaluator.java`, emitting to `traverse.analysis.results`. Registered in `ensure_flink_jobs.py`. `GET /analyses/types` fixed: the misleading "Custom Flink SQL" type (nothing executed it) is replaced by an accurate `calculation` type.
- **Publish to UNS (7.3):** analysis-service `/execute` sources each input's live value from Redis; a new `AnalysisResultConsumer` reads `traverse.analysis.results`, closes the execution, writes the derived value to the live plane, and **registers the derived measurement in asset-model** so binding-resolver resolves it and any symbol can bind it like a tag.
...
| 7.2 | **Flink job** consuming `traverse.analysis.executions` + `ExpressionEvaluator` + fixed `GET /analyses/types` | L12, L13, Discrepancy §7.3 | ✅ (not mvn-compiled) |
...
**Audit check:** grep confirms **no** `NCalc|Jint|ExpressionParser` added to the frontend (calc stays server/Flink-side); a Flink job now consumes `traverse.analysis.commands` (audit found 0 consumers); `binding.resolve` is no longer flat for Viewer.
