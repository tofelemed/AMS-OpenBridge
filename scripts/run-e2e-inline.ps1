...
Test-Step "AMS API health" { $r = Invoke-RestMethod http://localhost:8000/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Historian BFF health" { $r = Invoke-RestMethod http://localhost:8090/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Asset Model health" { $r = Invoke-RestMethod http://localhost:5001/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Binding Resolver health" { $r = Invoke-RestMethod http://localhost:5002/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Display Service health" { $r = Invoke-RestMethod http://localhost:5003/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Template Service health" { $r = Invoke-RestMethod http://localhost:5004/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }
Test-Step "Analysis Service health" { $r = Invoke-RestMethod http://localhost:5005/health -TimeoutSec 10; if ($r.status -ne 'Healthy') { throw $r.status } }

Test-Step "List assets (seed data)" { $r = Invoke-RestMethod http://localhost:5001/assets -TimeoutSec 10; if ($r.total -lt 5) { throw "expected >=5 got $($r.total)" } }
Test-Step "List displays" { $r = Invoke-RestMethod http://localhost:5003/displays -TimeoutSec 10; if ($r.total -lt 1) { throw "expected >=1" } }
Test-Step "List templates" { $r = Invoke-RestMethod http://localhost:5004/templates -TimeoutSec 10; if ($r.total -lt 2) { throw "expected >=2" } }
Test-Step "List analyses" { $r = Invoke-RestMethod http://localhost:5005/analyses -TimeoutSec 10; if ($r.total -lt 3) { throw "expected >=3" } }
...
Test-Step "Asset create + bind + delete" {
    ...
    $asset = Invoke-RestMethod http://localhost:5001/assets -Method Post -Body $body -ContentType "application/json" -TimeoutSec 10
    $url = "http://localhost:5002/resolve?path=$path" + "&roles=all"
    $bind = Invoke-RestMethod $url -TimeoutSec 10
    if (-not $bind.resolved) { throw "not resolved" }
    Invoke-RestMethod "http://localhost:5001/assets/$($asset.id)" -Method Delete -TimeoutSec 10 | Out-Null
}
...
Test-Step "Designer proxy - create display" {
    ...
    Invoke-RestMethod "http://localhost:5003/displays/$($r.id)" -Method Delete -TimeoutSec 10 | Out-Null
}

Test-Step "Historian snapshot (live data path)" {
    $r = Invoke-RestMethod "http://localhost:8090/snapshot?assets=*" -TimeoutSec 10
