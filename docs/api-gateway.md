...
| `/api/ingestion/**` | ingestion-service :5000 | strip `/api/ingestion` → `/data-sources…`, `/profiles`, `/stats`, `/unknown-sources` | required | OT data-source configs + phase-2 subscriber ops (stats, parking inventory, reload) — `ingestion.view`/`ingestion.manage`, Admin-only |
