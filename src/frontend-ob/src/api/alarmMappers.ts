...
export function mapHistoricalAlarmRow(dto: Record<string, unknown>): Record<string, unknown> {
  const eventTimeEpochMs = toEpochMs(pick(dto, 'eventTime', 'EventTime'));
  const ackTimeEpochMs = toEpochMs(pick(dto, 'ackTime', 'AckTime'));
  
  return {
    id: String(pick(dto, 'id', 'Id') ?? ''),
    serverId: String(pick(dto, 'serverId', 'ServerId') ?? ''),
    serverName: String(pick(dto, 'serverName', 'ServerName') ?? ''),
    sourceName: String(pick(dto, 'sourceName', 'SourceName') ?? ''),
    conditionName: (pick(dto, 'conditionName', 'ConditionName') as string | null) ?? '',
    subConditionName: (pick(dto, 'subConditionName', 'SubConditionName') as string | null) ?? '',
    message: (pick(dto, 'message', 'Message') as string | null) ?? '',
    severity: Number(pick(dto, 'severity', 'Severity') ?? 0),
    priority: String(pick(dto, 'priorityLabel', 'PriorityLabel', 'priority', 'Priority') ?? 'LOW').toUpperCase(),
    category: String(pick(dto, 'category', 'Category') ?? 'PROCESS').toUpperCase(),
    state: String(pick(dto, 'stateLabel', 'StateLabel', 'state', 'State') ?? 'UNKNOWN').toUpperCase(),
    eventTimeEpochMs: eventTimeEpochMs ?? 0,
    ackTimeEpochMs: ackTimeEpochMs,
    ackedByUsername: (pick(dto, 'ackedByUsername', 'AckedByUsername') as string | null) ?? '',
    acknowledged: Boolean(pick(dto, 'acknowledged', 'Acknowledged') ?? false),
  };
}
