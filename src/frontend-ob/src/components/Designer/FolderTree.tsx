        ...
        >
          {node.children.length > 0 ? (
            <button className="ft__caret" onClick={(e) => { e.stopPropagation(); setExpanded(s => { const n = new Set(s); n.has(node.id) ? n.delete(node.id) : n.add(node.id); return n; }); }}>
              {isOpen ? <ObiChevronDownGoogle /> : <ObiChevronRightGoogle />}
            </button>
          ...
          <span className="ft__actions">
            ...
            <button title="Rename" onClick={async (e) => { e.stopPropagation(); const name = await prompt({ title: 'Rename folder', label: 'Folder name', defaultValue: node.name }); if (name?.trim() && name !== node.name) renameFolder.mutate({ id: node.id, name: name.trim(), parentId: node.parentId }); }}><ObiEditGoogle /></button>
            <button title="Delete (displays inside are kept, just unfiled)" onClick={async (e) => { e.stopPropagation(); if (await confirm({ title: 'Delete folder', message: `Delete folder "${node.name}"? Displays inside are moved to Unfiled.`, confirmLabel: 'Delete', danger: true })) deleteFolder.mutate(node.id); }}><ObiDelete /></button>
          </span>
        </div>
