...
/** Fast KPI bar from server-side aggregates (no full alarm scan). */
...
/** Load active alarms in pages without blocking the SignalR ping loop. */
...
export async function fetchAllActiveAlarms(
  ...
  while (all.length < totalCount) {
    ...
    // Yield so SignalR/WebSocket keepalive can run between heavy pages.
...
/** HTTP alarm feed — sole connected alarm source. */
...
/** Remove storm/autonomous lab rows from active_alarms DB. */
