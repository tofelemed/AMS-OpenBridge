...
// Enable Immer Map/Set support — required for alarm Map and selectedAlarmIds Set
enableMapSet();
...
export interface ActiveAlarm {
  ...
  /** Cross-version KPI/SOE family — stable when instance key formula changes (production-contracts §1). */
  logicalAlarmFamilyId: string;
  instanceKeySchemaVersion: number;
  /** True when REST/hub payload lacked DCS eventTime (audit flag). */
  eventTimeMissing?: boolean;
  /** ACK orchestration lifecycle (ACK_REQUESTED … ACK_CONFIRMED) */
...
// ============================================================
// Alarm Store (Zustand with Immer)
// ============================================================
...
interface AlarmStore {
  ...
  /** OPC-AE server IDs currently connected via gateway (lab: IO simulator). */
...
function recalcStatsFromAlarms(alarms: Map<string, ActiveAlarm>, connectedOpcServerIds: Set<string>): AlarmStats {
  ...
  // OPC A&E: only conditionActive=true alarms count as "active"
...
async function syncConnectedOpcServers(set: (fn: (state: AlarmStore) => void) => void): Promise<{ id?: string; protocol?: string }> {
  try {
...
async function hydrateAlarmsFromApi(
  set: (fn: (state: AlarmStore) => void) => void,
  opts: HydrateOptions = {},
) {
  ...
  try {
    ...
    /* KPI bar falls back to client-side recalc as pages arrive */
  }
  ...
  console.info(`[AlarmStore] Loaded ${stored}/${fetched} simulator alarms from API`);
}
  ...
  immer((set, get) => ({
    ...
    initialize: async (token: string) => {
      ...
      hubInitInFlight = (async () => {
        ...
        connection.on('OnConnected', () => { /* hub handshake — suppress client warning */ });
        connection.on('onconnected', () => { /* legacy/camelCase hub name */ });
        ...
        try {
          ...
          // Background load — must not block WS ping/keepalive (prevents 1011 disconnects).
    ...
    bulkUpdateAlarms: (ids, action) => {
      set(state => {
        for (const id of ids) {
          const alarm = state.alarms.get(id);
          if (!alarm) continue;
          // ACK confirmation only via applyAckLifecycle (Flink → lifecycle-events → SignalR).
    ...
    setLoopKpi: (payload) => {
      set(state => {
        state.loopKpis[payload.tagId] = payload;
      });
    },

    setAlarmKpi: (payload) => {
      set(state => {
        state.alarmKpis[payload.kpiType] = payload;
      });
    },
...
function playAlarmSound(priority: string) {
  ...
  } catch (e) { /* Audio may be blocked by browser policy */ }
}
