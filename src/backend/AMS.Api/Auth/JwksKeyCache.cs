...
///
/// This replaces the former <c>TestAuthHandler</c>, which authenticated EVERY anonymous request with a
/// full permission set (including admin.*), making every <c>[Authorize(Policy = ...)]</c> on this API a
/// no-op. auth-service serves JWKS but no OIDC discovery document, so JwtBearer's Authority/metadata
/// flow can't be used; we fetch the key set directly and refresh it when a token presents an unknown
/// <c>kid</c> (i.e. after auth-service rotates its keypair).
/// </summary>
public sealed class JwksKeyCache
{
    ...
    public async Task RefreshAsync(CancellationToken ct = default)
    {
        ...
        {
            // A transient auth-service outage must not 500 the pipeline: with no keys, validation fails
            // and the caller simply gets 401 until JWKS is reachable again.
