...
| P2-21 | FIXED — auto-commit off; offsets committed per-partition only after MQTT delivery. `publishMqtt` throws when disconnected (was log-and-drop); poll loop pauses consumption while EMQX is down (group membership kept alive), seeks back and replays on delivery failure, skips only genuinely poison records. Validated live with a real EMQX outage: delivery failure at `traverse.live.metrics:2` offset 9269028 logged "will replay after reconnect", committed offsets froze on every partition while log-end grew (lag 3815→4160+), reconnect at broker return logged "resuming Kafka consumption", backlog drained, snapshot went fresh — zero records lost |
