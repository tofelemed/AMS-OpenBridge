# v2.4 deployment — CHG-023 (CPM Overview / Performance read path)

Ships **cplm-api, ams-frontend, historian-bff, binding-resolver, traverse-auth-service**
(CHG-023 + CHG-024 + CHG-025). Three additive indexes, applied **online before the image
swap**. No Flink change, no job resubmission, no data load. Companion:
[UPDATE-RUNBOOK.md](UPDATE-RUNBOOK.md); the change itself is CHG-023 in
[changes_tracker.md](../changes_tracker.md).

| | |
|---|---|
| Bundle | `release-out/v2.4-<date>/` from `python migration/deploy/build-release.py --release v2.4` |
| Services | `cplm-api`, `ams-frontend`, `historian-bff`, `binding-resolver`, `traverse-auth-service` |
...
dotnet test tests/cplm-api.Tests                     # 43/43 (lab Postgres on 5433; ~7 min, the legacy oracle is that slow)
...
cd src/frontend-ob; npm test; npm run lint; npm run build; cd ..\..   # 22/22
cd src/services/auth-service; npx tsc --noEmit; cd ..\..\..
python migration/deploy/build-release.py --release v2.4 --dry-run
python migration/deploy/build-release.py --release v2.4
```

## B. Transfer

As V3: `scp -r release-out\v6-<date> lean@192.168.190.91:/tmp/v2.4`, then
`sudo cp -r /tmp/v2.4 /opt/ams-recovery/`.

## C. Plant

```bash
cd /tmp/v2.4 && sha256sum -c SHA256SUMS.txt                          # every line OK
docker images --no-trunc --format '{{.ID}} {{.Repository}}:{{.Tag}}' > /tmp/pre-v2.4-images.txt
...
# C3. images
for f in *.tar.gz; do gunzip -c "$f" | docker load; done      # 5 images
...
images from `/tmp/pre-v2.4-images.txt` and `deploy.sh --prod`; the old queries still work, only
slowly.
