#!/usr/bin/env python3
"""Full ACK loop: operator-actions -> OpcEventStreamJob -> ack-writeback ->
HttpAckWritebackService -> external ACK URL -> ack-results -> OpcEventStreamJob
-> current-alarm-state (ACK_STATE_UPDATE) -> Postgres -> SignalR.

Two modes:
  --via-api (default): seeds fresh alarms, then POSTs the real
      /api/v1/alarms/acknowledge/batch so the API's operator-actions publish and
...
then injects a synthetic ACK_CONFIRMED into ack-results (mimicking the DCS
...
def seed_alarms(tag: str, count: int) -> dict[str, dict]:
    ...
    sl.kafka_publish_batch("raw-alarms", records)
    ok = sl.wait_until(
...
def main() -> int:
    ...
    awb_before = sl.kafka_end_offset_sum("ack-writeback")
    ares_before = sl.kafka_end_offset_sum("ack-results")
    ...
    if args.via_api:
        ...
        # ack-results — which only the writeback service produces).
        time.sleep(2)
        mid = sl.pg_query(
            f"SELECT count(*) FROM alarms.alarm_current WHERE alarm_id LIKE 'ACK-{tag}-%'"
            " AND ack_status")
        if int(mid[0][0]) == 0:
            ...
            early = sl.kafka_consume("ack-results", max_messages=3000, timeout_sec=20,
        ...
        sl.kafka_publish_batch("operator-actions", actions, interval=args.interval)
        sl.log(f"injected {len(actions)} operator-actions")

    # 1. Flink ack-processor -> ack-writeback (ACK_DISPATCHED)
    checks["ack_writeback_grew"] = sl.wait_until(
        "ack-writeback to grow",
        lambda: sl.kafka_end_offset_sum("ack-writeback") > awb_before, timeout_sec=90)
    awb = sl.kafka_consume("ack-writeback", max_messages=2000, timeout_sec=20,
                           contains=f"AckSim.{tag}.")
    ...
    # 2. HttpAckWritebackService -> ack-results (ACK_FAILED expected: PIPE-002)
    checks["ack_results_grew"] = sl.wait_until(
        "ack-results to grow",
        lambda: sl.kafka_end_offset_sum("ack-results") > ares_before, timeout_sec=180)
    ares = sl.kafka_consume("ack-results", max_messages=2000, timeout_sec=20,
                            contains=f"AckSim.{tag}.")
    ...
    # 3. lifecycle-events carries the result state transitions
    lce = sl.kafka_consume("lifecycle-events", max_messages=5000, timeout_sec=20,
                           contains=f"AckSim.{tag}.")
    ...
    # 4. synthetic DCS confirm -> ack-results ACK_CONFIRMED -> ACK_STATE_UPDATE -> Postgres
    confirms = []
    ...
    sl.kafka_publish_batch("ack-results", confirms)
    sl.log("injected synthetic ACK_CONFIRMED results (mimicking DCS reply)")

    checks["ack_confirm_reached_cas"] = sl.wait_until(
        "ACK_STATE_UPDATE on current-alarm-state",
        lambda: sl.kafka_end_offset_sum("current-alarm-state") > cas_before, timeout_sec=90)
    cas = sl.kafka_consume("current-alarm-state", max_messages=8000, timeout_sec=25,
                           contains=f"ACK-{tag}-")
