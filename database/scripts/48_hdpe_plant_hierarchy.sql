-- =============================================================================
-- 48_hdpe_plant_hierarchy.sql - the real HDPE plant tree (P6, MIGRATION_LOG #17)
--
-- Site -> Area -> Unit rows for the HDPE plant, generated from the engineering
-- handoff (hirarichy.csv, 2026-08-24): 1 site, 7 areas (+Unassigned), 25 units.
-- Path segments are lowercase snake_case slugs - unit slugs carry a 'u' prefix
-- because IoTDB path nodes must not start with a digit (the same constraint
-- IotDbWriteClient.SafeNode guards). Human labels live in name; the origin
-- Instrumental-Pro IDs stay resolvable via assets.alias_mapping rows with
-- source_system = 'instrumental-pro'.
--
-- Additive + idempotent (ON CONFLICT DO NOTHING throughout).
-- Apply to a running DB:  psql -U ams_user -d traverse_assets -f 48_... .sql
-- =============================================================================
\c traverse_assets
...
-- The origin system's natural keys ('HDPE', 'Section 100', '1001-Polymerization
-- Reactor 1') and its full backslash paths keep resolving, so CSVs exported
-- from it import against this tree unchanged.
